<?php

namespace App;

use Illuminate\Database\Eloquent\Model;
use IImage;
use Request;
use File;

class Image extends Model
{
    public $saveTo = 'images';

    /**
     * define the fillable field
     *
     * @var array
     */
    protected $fillable = [
        'reference_id',
        'path',
        'width',
        'height'
    ];

    protected $table = 'images';

    /**
     *
     *
     * @param $reference_id
     * @param $width
     * @param $height
     * @return null
     */
    public static function findSimilarOrCreate($reference_id, $width, $height) {
        $images = Image::where('reference_id', $reference_id)->get();
        if ($images->count() == 0) {
            return null;
        } else {
            $original_image = Image::findOrFail($reference_id);
            // try to find similar image size
            $candidates = $images->filter(function($image) use ($width,$height) {
                return $image->isSimilar($width, $height);
            });

            if ($candidates->count() > 0) {
                // we have find similar image size
                $image = $candidates->first();
                return $image->toIImage();
            } else {
                // we don't have desired image size, let use create one and store it.
                $img = IImage::make(base_path($original_image->path))->fit($width, $height);
                // get path and name
                $path_name = explode('.', $original_image->path)[0];
                $extension = explode('.', $original_image->path)[1];
                // generate file name
                $file_name = $path_name . '_' . $width . 'x' . $height . '.' . $extension;
                $img->save(base_path($file_name));
                // store it in the database
                $newImg = Image::create([
                    'reference_id' => $reference_id,
                    'path' => $file_name,
                    'width' => $width,
                    'height' => $height
                ]);
                $newImg->save();
                return $img;
            }

        }
    }

    /**
     * To Intervention image instance
     */
    public function toIImage() {
        return IImage::make(base_path($this->path));
    }

    /**
     * determine if the ratio of request image is similar
     *
     * @param $width
     * @param $height
     * @return bool
     */
    public function isSimilar($width, $height) {
        $width_diff = abs($this->width - $width);
        $width_diff_ratio = $width_diff / $this->width;

        $height_diff = abs($this->height - $height);
        $height_diff_ratio = $height_diff / $this->height;

        return ($width_diff_ratio < 0.3) && ($height_diff_ratio < 0.3);
    }


    /**
     * All the picture with all different size
     *
     * @return bool|null
     * @throws \Exception
     */
    public function deleteAll() {
        $images = Image::where('reference_id', $this->reference_id)->get();
        foreach ($images as $image) {
            $image->delete();
        }
    }

    /**
     * Override delete method so that it can delete file
     *
     * @return bool|null
     * @throws \Exception
     */
    public function delete() {
        File::delete($this->path);

        return parent::delete(); // TODO: Change the autogenerated stub
    }

    /**
     * determine `src` is from this site
     *
     * @param $src
     * @return bool
     */
    public static function isFromThisSite($src) {
        $parse = parse_url($src);
        $root = parse_url(Request::root());
        if (!isset($parse['host'])) {
            return false;
        }

        if ($parse['host'] != $root['host']) {
            return false;
        }

        // not begin with `/image/`
        if (strpos($parse['path'], '/image/') != 0) {
            return false;
        }

        // not find image id
        $id = explode('/', $parse['path'])[2];
        if (!Image::where('id', $id)->exists()) {
            return false;
        }

        return Image::findOrFail($id);
    }

}
